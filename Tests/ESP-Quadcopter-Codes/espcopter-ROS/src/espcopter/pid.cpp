...
void PID::compute(int whichIs, int32_t TARGET_ANGLE, int32_t THROTTLE, int32_t TRIM, int32_t MEASURED_ANGLE, int32_t MEASURED_RATE ) {
  ...
  if (dt > -1) {
    ...
    // if( whichIs == 6 || whichIs == 7 ){
    // itermRate= constrain(itermRate, -20, 20);
    //}

    // if(whichIs = 3){
    // itermRate= constrain(itermRate, -5, 5);
    // }
    ...
    */
    ...
    D = (KDO / 100) * delta;
    /*
       if(whichIs == 4  && (MEASURED_RATE < -25000 || MEASURED_RATE > 25000) ){ // TARGET_ANGLE != 0
        P=0;
        D=0;
       }else if(whichIs == 5  && (MEASURED_RATE < -25000 || MEASURED_RATE > 25000) ){ //
        P=0;
        D=0;
       }else{ */
    ...
    // if( whichIs == 6 && whichIs == 7 && THROTTLE < 100 ){
    ...
    /*
        ...
        if(RATE_ERROR < 0){
        KIO = map(RATE_ERROR, -250,0,25,15 )/10;
        }else{
        KIO = map(RATE_ERROR, 0 ,250,15,25 )/10;
        }
       }*/
