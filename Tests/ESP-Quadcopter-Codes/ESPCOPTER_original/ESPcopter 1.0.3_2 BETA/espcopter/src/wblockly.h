...
String AP_PassString = "123456789";
 ...
 const char *MeteAp = "ESPcopter"; 
 const char *MetePass = "123456789";

const char *OTAName = "ESPcopter";           // A name and a password for the OTA service
const char *OTAPassword = "espcopter";
...
const char* mdnsName = "espcopter"; // Domain name for the mDNS responder


 // WiFiClient client2; 
 // WiFiServer server2(6377);
...
String mainOutput= "";
    ...
    mainOutput = mainOutput + getBatteryVolt() + '\n';
...
void sendBattery() {
  server_1.send(200, "text/plane", String(getBatteryVolt())); //Send ADC value only to client ajax request ??
}
