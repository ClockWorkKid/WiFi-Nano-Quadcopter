...
float alttidue = constrain(otoMeasure, 300, 700); // constrain(otoMeasure, 100, 1000); 
  ...
  float deltaCalX_;
  /*
  deltaCalX = (0.5*(float)deltaCalX) + ((1-0.5)*(float)deltaCalX_);
  deltaCalX_ = deltaCalX;

   deltaCalY = (0.5*(float)deltaCalY) + ((1-0.5)*(float)deltaCalY_);
   deltaCalY_ = deltaCalY;
*/
   ...
   trimPlusY = 1 + map(abs(yOpt.GetItermRateBase()),0,6000, 0, 2);
   }else{
   trimPlusY = 1 - map(abs(yOpt.GetItermRateBase()),0,6000, 0, 0.5);
   }
   }else{
   if( SetPointOpt[1] + RX_pitch_Multi > 0 ){
   trimPlusY = 1 - map(abs(yOpt.GetItermRateBase()),0,6000, 0, 0.5);
   ...
   trimPlusX = 1 + map(abs(xOpt.GetItermRateBase()),0,5000, 0, 2);
   }else{
   trimPlusX = 1 - map(abs(xOpt.GetItermRateBase()),0,5000, 0, 0.5);
   }
   }else{
   if( SetPointOpt[0] + RX_roll_Multi > 0 ){
   trimPlusX = 1 - map(abs(xOpt.GetItermRateBase()),0,5000, 0, 0.5);
  ...
  if(checkEepromOpt == 1 && throttle < 100 ){
