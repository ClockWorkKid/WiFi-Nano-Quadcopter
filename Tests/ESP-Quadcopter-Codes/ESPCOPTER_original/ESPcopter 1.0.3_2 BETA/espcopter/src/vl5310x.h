...
void vl5310x(){
  ...
  //otoMeasure = otoMeasure - triOtoMeasure;
   ...
   otoMeasure = (0.6*(float)otoMeasure) + ((1-0.6)*(float)otoMeasure_0); //0.65
  ...
  hoverFactor_ = map(5.5*analogRead(A0),3600,4200,550+otoNewSet,450+otoNewSet) * weightOfDrone; //6*analogRead(A0)
  ...
  checkEepromOto =1;
   ...
   if(otoIntervalCont == takeOffTimeSecond){ //35  
     if(otoMeasure < 250 || 750 < otoMeasure  ){
      stopFlightControl = 0;
    }
  }
