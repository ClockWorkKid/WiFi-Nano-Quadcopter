...
void AHRS::setTrimRoll(int16_t trimRoll_){
   trimRoll =  trimRoll_;
}
void AHRS::setTrimPitch(int16_t trimPitch_){
   trimPitch =  trimPitch_;
}
...
void AHRS::compute(float attitude[3], float rate[3],float attitudeRadian[3], float rateRadian[3] ){
    ...
    I2Cread(MPU9250_ADDRESS,0x3B,14,Buf);
    accel[0]=(Buf[0]<<8 | Buf[1]) - MEAN_ACCE[0]; //-520
    accel[1]=(Buf[2]<<8 | Buf[3]) - MEAN_ACCE[1]; //-300
    accel[2]=(Buf[4]<<8 | Buf[5]) - MEAN_ACCE[2];
...
/*
   ...
   Serial.print("  ,  ");
   Serial.print(previousHeadingDegrees - headingDegrees );
   Serial.print("  ,  ");
