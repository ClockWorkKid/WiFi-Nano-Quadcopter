...
void mainSetup(){
 ...
 Serial.println(" done Trim_Bs");
   ...
   if(Trim_IMU == 1){
   EEPROM.write(66,highByte(Trim_Roll_IMU));
   EEPROM.write(67,lowByte(Trim_Roll_IMU));

   EEPROM.write(68,highByte(Trim_Pitch_IMU));
   EEPROM.write(69,lowByte(Trim_Pitch_IMU));
    
  EEPROM.commit();
  }

  Trim_Roll_IMU = word(EEPROM.read(66),EEPROM.read(67));
  Trim_Pitch_IMU = word(EEPROM.read(68),EEPROM.read(69));

  ahrs.setTrimRoll(Trim_Roll_IMU);
  ahrs.setTrimPitch(Trim_Pitch_IMU);
  ...
  //calibrationInt();
   ...
   roll.SetGain(15.2, 25.0, 0.18,0.0,1.4);//    roll.SetGain(7.2, 26.0, 0.18,0.6,0.9);
   roll.SetLimit(4500, 3000, 6000);  //  roll.SetLimit(4500, 1000, 3500); 
   
   pitch.SetGain(15.2, 25.0, 0.18,0.0,1.4); //    pitch.SetGain(15.2, 26.0, 0.18,0.0,1.4);
